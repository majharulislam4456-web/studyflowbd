...
const navItems = [
  ...
  { id: 'timer', label: 'Timer', labelBn: 'টাইমার', icon: Timer },
  { id: 'goals', label: 'Goals', labelBn: 'লক্ষ্য', icon: Target },
  { id: 'todos', label: 'To-Do', labelBn: 'করণীয়', icon: ListTodo },
...
export function AppSidebar({ activeTab, setActiveTab, isDark, toggleTheme, isCollapsed, setIsCollapsed, profile }: AppSidebarProps) {
  const getInitials = () => {
      ...
      <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto scrollbar-hide">
        {navItems.map(item => (
          <button key={item.id} onClick={() => setActiveTab(item.id)}
            ...
            <item.icon className={cn("w-[18px] h-[18px] flex-shrink-0", activeTab !== item.id && "group-hover:text-primary")} />
      ...
      <div className="px-3 py-3 border-t border-sidebar-border space-y-1">
        <button onClick={() => setActiveTab('settings')}
          className={cn(
            ...
            activeTab === 'settings' ? "bg-primary text-primary-foreground shadow-sm" : "text-sidebar-foreground/70 hover:bg-sidebar-accent hover:text-sidebar-foreground"
          )}>
        ...
        <div className="flex items-center gap-1">
          <Button variant="ghost" size="icon" onClick={toggleTheme} className="flex-1 h-9 text-muted-foreground hover:text-foreground">
            {isDark ? <Sun className="w-4 h-4" /> : <Moon className="w-4 h-4" />}
          </Button>
          <Button variant="ghost" size="icon" onClick={() => setIsCollapsed(!isCollapsed)} className="h-9 text-muted-foreground hover:text-foreground">
